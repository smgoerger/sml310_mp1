...
shaq_modelComplete <- stan_model(model_code = shaq_model_stanComplete, model_name = "shaq_modelComplete")
shaqComplete_fit <- sampling(object=shaq_modelComplete,
                      data = list(N=nrow(shaq), scored=shaq$Scored, attempted = shaq$N.Attempts),
                      chains = nChains ,
                      iter = ( ceiling(numSavedSteps/nChains)*thinSteps
                               +burnInSteps ) , 
...
par(mfrow=c(2, 2))
hist(samplesComplete$mu, main = "mu")
# hist(sigma)
barplot(length(sigma), sigma, main = "sigma", names.arg = "0")
hist(samplesComplete$alpha[, 1], main =  "game 1")
hist(samplesComplete$alpha[, 2], main = "game 2") # they're the same
@
