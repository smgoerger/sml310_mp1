...
y <- rbinom(N.p2, size = 1, prob = plogis(x2 - 6))
hist(y)
...
hist(personality)
#not get into harvard?
harvard <- rbinom(N.p2, size = 1, prob = plogis(personality - 6))
hist(harvard)
...
<<p3bHist>>=
p.3og.mu <- hist(samples$mu)
p.3og.sigma <- hist(samples$sigma)
p.3og.alpha <- hist(samples$alpha_norm)
